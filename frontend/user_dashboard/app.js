...
// Single-page app: auth, registration, email-based OTP, device management,
// emergency contacts, preferences, incident history.
//
// OTP delivery: verification codes are generated by the backend and queued
// in Postgres (email_queue table). An external Google Apps Script polls
// GET /email-queue/pending and sends the actual email via GmailApp — this
// frontend never talks to an SMS/email provider directly.
// ─────────────────────────────────────────────────────────────────────────────
...
function startOtpView(email, hintText) {
  document.getElementById("otp-hint").textContent = `${hintText} — code sent to ${email}`;
